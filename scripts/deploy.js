...
async function main() {
  try {
    ...
    const Pepper = await ethers.getContractFactory("Pepper");
    
    // Check contract size
    const bytecode = Pepper.bytecode;
    ...
    // Estimate gas
    const estimatedGas = await Pepper.signer.estimateGas(
      Pepper.getDeployTransaction(
    ...
    const Pepper = await Pepper.deploy(
      250, // Platform fee (2.5%)
      ethers.utils.parseEther("10"), // Minimum bet amount (10 CHZ)
      deployer.address, // Use deployer as treasury for now
      { 
    ...
    await Pepper.deployed();

    console.log("Pepper deployed to:", Pepper.address);
    
    // Log final cost
    const deploymentReceipt = await Pepper.deployTransaction.wait();
